...
// Find start/end of the C palette object
...
// ── 3. Add $effect for theme re-render (after the $derived line) ───────────────
rep(
...
// ── 4. Add ROOM_DESC + getRoomDescription after the flavor text arrays ─────────
// Insert before the trap tables comment
...
// ── 7. Update encounterListContent snippet – make room number a button ─────────
// Find and replace the each block inside the snippet
const oldEach = `\t\t\t{#each floorEncounters[currentFloor].slice(1) as enc, i}
\t\t\t\t{@const treasure = floorTreasure[currentFloor]?.[i + 1] ?? ''}
\t\t\t\t<div class="flex flex-col gap-1 border-b border-white/5 px-3 py-2">
\t\t\t\t\t<div
\t\t\t\t\t\tclass="flex gap-2 {enc === 'Empty' && !treasure
\t\t\t\t\t\t\t? 'text-gray-600'
\t\t\t\t\t\t\t: 'text-gray-200'}"
\t\t\t\t\t>
\t\t\t\t\t\t<span class="w-5 shrink-0 font-bold text-indigo-400">{i + 1}</span>
...
const newEach = `\t\t\t{#each floorEncounters[currentFloor].slice(1) as enc, i}
\t\t\t\t{@const roomId = i + 1}
...
rep('encounterListContent each', oldEach, newEach);

// ── 8. Add theme selector to options panel (after Floors, before Boss Room) ────
const floorsLabelEnd = `\t\t</label>\n\n\t\t<label class="flex cursor-pointer items-center gap-2">\n\t\t\t<input type="checkbox" bind:checked={includeBossRoom}`;
const withTheme = `\t\t</label>\n\n\t\t<label class="flex flex-col gap-1">\n\t\t\tTheme\n\t\t\t<select bind:value={dungeonTheme} class="w-full rounded bg-gray-700 px-1 py-0.5 text-white">\n\t\t\t\t<option value="Crypt">⚰ Crypt</option>\n\t\t\t\t<option value="Sewer">🪣 Sewer</option>\n\t\t\t\t<option value="Cave">🪨 Cave</option>\n\t\t\t\t<option value="Fortress">🏰 Fortress</option>\n\t\t\t\t<option value="Arcane">🔮 Arcane</option>\n\t\t\t\t<option value="Fungal">🍄 Fungal</option>\n\t\t\t</select>\n\t\t</label>\n\n\t\t<label class="flex cursor-pointer items-center gap-2">\n\t\t\t<input type="checkbox" bind:checked={includeBossRoom}`;
rep('theme selector in options', floorsLabelEnd, withTheme);

// ── 9. Update legend swatches to use C.* dynamic colors ───────────────────────
rep(
	'legend Room swatch',
	`<span class="inline-block h-3 w-3 rounded-sm bg-[#2a3a4a] align-middle"></span> Room</span>`,
	`<span class="inline-block h-3 w-3 rounded-sm align-middle" style="background:{C.floor}"></span> Room</span>`
);
rep(
	'legend Corridor swatch',
	`<span class="inline-block h-3 w-3 rounded-sm bg-[#1e2c3a] align-middle"></span> Corridor</span>`,
	`<span class="inline-block h-3 w-3 rounded-sm align-middle" style="background:{C.corridor}"></span> Corridor</span>`
);
rep(
	'legend Door swatch',
	`<span class="inline-block h-3 w-3 rounded-sm bg-[#c87820] align-middle"></span> Door</span>`,
	`<span class="inline-block h-3 w-3 rounded-sm align-middle" style="background:{C.door}"></span> Door</span>`
);
rep(
	'legend Locked/Trapped swatch',
	`<span class="inline-block h-3 w-3 rounded-sm bg-[#cc2222] align-middle"></span> Locked/Trapped</span>`,
	`<span class="inline-block h-3 w-3 rounded-sm align-middle" style="background:{C.trap}"></span> Locked/Trapped</span>`
);
rep(
	'legend Secret swatch',
	`<span class="inline-block h-3 w-3 rounded-sm bg-[#1a2a5a] align-middle"></span> Secret</span>`,
	`<span class="inline-block h-3 w-3 rounded-sm align-middle" style="background:{C.wallDim}"></span> Secret</span>`
);
rep(
	'boss room checkbox swatch',
...
// ── 10. Add room description popover template (before trap popup) ─────────────
rep(
	'room desc popover',
	`\t<!-- trap detail popup -->`,
...
\t\t\tstyle="left: {Math.min((activeRoomPopover.anchorEl.getBoundingClientRect().left - 300), window.innerWidth - 310)}px; top: {Math.min(activeRoomPopover.anchorEl.getBoundingClientRect().top, window.innerHeight - 200)}px;"
\t\t>
...
\t\t\t\t\tRoom {activeRoomPopover.roomId}{activeRoomPopover.isBoss ? ' · Boss' : activeRoomPopover.isStart ? ' · Start' : ''}
\t\t\t\t</h3>
...
\t\t\t</p>
\t\t\t<div class="mt-3 text-[10px] uppercase tracking-wide text-gray-600">{dungeonTheme} · for players</div>
\t\t</div>
\t{/if}
...
// ── 11. Add dungeonTheme to DungeonSave settings ──────────────────────────────
rep(
	'DungeonSave settings type',
	`\t\t\tnumFloors: number;`,
	`\t\t\tnumFloors: number;\n\t\t\tdungeonTheme?: DungeonTheme;`
);
rep(
	'confirmSave settings object',
...
rep(
	'loadSave dungeonTheme restore',
	`\t\tdungeonTheme = entry.settings.dungeonTheme ?? 'Crypt';`,
	`\t\tdungeonTheme = entry.settings.dungeonTheme ?? 'Crypt';`
);
// Add it if not already there
if (!c.includes(`dungeonTheme = entry.settings.dungeonTheme`)) {
	rep(
		'loadSave dungeonTheme',
