...
<div class="flex h-screen flex-col overflow-hidden bg-gray-950 text-white">
	...
	<header class="flex shrink-0 items-center border-b border-gray-800 bg-gray-900 px-6 py-3">
		...
		<div class="ml-6 flex items-center gap-2 rounded-lg border border-gray-700 bg-gray-800/60 px-3 py-1.5">
			<span class="hidden text-xs text-gray-500 uppercase tracking-widest sm:inline">Session</span>
			<span class="max-w-[120px] truncate text-sm font-semibold text-amber-300">{activeSession.name}</span>
					...
					>
						...
						<div class="min-w-0 flex-1">
							...
							{:else}
								...
								>
									<span class="block truncate text-sm font-medium {isActive ? 'text-amber-300' : 'text-gray-200 group-hover:text-white'}">
										{session.name}
									</span>
									<span class="font-mono text-xs tracking-widest {isActive ? 'text-amber-500/70' : 'text-gray-500'}">
										{session.sessionId}
									</span>
								</button>
							{/if}
