...
<script lang="ts">
	...
	const initResult = $derived.by(() => {
		...
		const chosen =
			initMode === 'advantage' ? Math.max(initRoll1, initRoll2) : Math.min(initRoll1, initRoll2);
		return chosen + dex;
	});
	...
	<div class="w-full max-w-sm rounded-xl border border-gray-700 bg-gray-900 shadow-2xl">
		...
		{:else}
			<div class="flex flex-col gap-5 px-5 py-5">
				...
				<div class="flex flex-col gap-1.5">
					...
					<div class="flex gap-2">
						{#each [{ value: 'normal', label: 'Normal' }, { value: 'advantage', label: 'Advantage' }, { value: 'disadvantage', label: 'Disadvantage' }] as opt}
							<label
								class="flex flex-1 cursor-pointer items-center justify-center gap-1.5 rounded-lg border px-2 py-2 text-xs font-semibold transition
								{initMode === opt.value
									? opt.value === 'advantage'
							...
							{#if initMode !== 'normal' && initRoll2 !== null}
								<div class="flex items-center gap-3">
									{#each [initRoll1, initRoll2] as roll, i}
										{@const isChosen =
											initMode === 'advantage'
												? roll === Math.max(initRoll1, initRoll2)
												: roll === Math.min(initRoll1, initRoll2)}
										<span
											class="text-3xl font-black {isChosen
												? 'text-amber-400'
												: 'text-gray-600 line-through'}">{roll}</span
										>
				...
				>
					{initSubmitting ? 'Submitting…' : 'Submit Initiative'}{#if initResult !== null}
						— {initResult}{/if}
				</button>
			</div>
		{/if}
