...
<script lang="ts">
	...
	function commitDamage(c: Combatant, sign: 1 | -1) {
		...
		damageInputs[c.id] = '';
		if (wasConcentrating) {
			concentrationCheck = { id: c.id, name: c.name, damage: val, dc: Math.max(10, Math.floor(val / 2)) };
		}
	}
					...
					{#if c.type === 'player' && c.currentHp <= 0}
						...
						<div class="rounded border border-gray-700 bg-gray-900/60 px-2 py-2 text-xs">
							<div class="mb-1.5 flex items-center gap-2">
								<span class="font-bold tracking-widest text-gray-400 uppercase">☠ Death Saves</span>
								<button
									onclick={() => combat.setDeathSaves(c.id, { successes: 0, failures: 0, stable: false })}
									class="ml-auto text-[10px] text-gray-600 hover:text-gray-400 transition"
									title="Reset death saves"
								>reset</button>
							</div>
							{#if isDead}
								<div class="flex items-center gap-2 rounded bg-red-950/60 px-2 py-1.5 text-sm font-bold text-red-400">
									<span>☠</span><span>Dead</span>
								</div>
							{:else if isStable}
								<div class="flex items-center gap-2 rounded bg-green-950/60 px-2 py-1.5 text-sm font-bold text-green-400">
									<span>♥</span><span>Stable</span>
								</div>
							{:else}
								<div class="flex items-center gap-4">
									<div class="flex flex-col gap-1">
										<span class="text-[10px] font-semibold tracking-wider text-red-500 uppercase">Failures</span>
									...
									<div class="flex flex-col gap-1">
										<span class="text-[10px] font-semibold tracking-wider text-green-600 uppercase">Successes</span>
										<div class="flex gap-1">
											{#each [0, 1, 2] as i}
												<button
													onclick={() => {
...
{#if concentrationCheck}
	...
	<div
		...
		onmousedown={(e) => { if (e.target === e.currentTarget) concentrationCheck = null; }}
	>
		<div class="mx-4 w-full max-w-sm rounded-xl border border-cyan-700/60 bg-gray-900 shadow-2xl shadow-cyan-900/30">
			<!-- Header -->
			<div class="flex items-center gap-2 border-b border-cyan-900/40 px-5 py-3">
				<span class="text-cyan-400">◈</span>
				<span class="text-sm font-bold tracking-widest text-cyan-300 uppercase">Concentration Check</span>
			</div>

			<!-- Body -->
			<div class="px-5 py-5">
				<p class="mb-1 text-base font-semibold text-white">{concentrationCheck.name}</p>
				<p class="mb-4 text-sm text-gray-400">
					took <span class="font-bold text-red-400">{concentrationCheck.damage}</span> damage
				</p>

				<!-- DC callout -->
				<div class="mb-4 flex items-center justify-between rounded-lg border border-cyan-800/50 bg-cyan-950/40 px-4 py-3">
					<div>
						<p class="text-xs font-semibold tracking-wider text-cyan-500 uppercase">CON Save DC</p>
						<p class="text-4xl font-bold text-cyan-300">{concentrationCheck.dc}</p>
					</div>
					<div class="text-right text-xs text-gray-500">
						<p>½ damage = {Math.floor(concentrationCheck.damage / 2)}</p>
						<p>minimum DC 10</p>
					</div>
				</div>

				<p class="text-xs text-gray-500">
					Fail → concentration ends. Success → spell continues.
				</p>
			</div>
			...
			<div class="flex gap-2 border-t border-gray-800 px-5 py-3">
				<button
					onclick={() => (concentrationCheck = null)}
				...
				<button
					onclick={() => {
						combat.toggleStatus(concentrationCheck!.id, 'Concentrating');
