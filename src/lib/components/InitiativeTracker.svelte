...
<ConcentrationCheckModal
	...
	onfail={(id) => { combat.toggleStatus(id, 'Concentrating'); concentrationCheck = null; }}
/>

<ConditionTimingModal
	pending={pendingCondition}
	onconfirmtimed={(rounds) => { combat.toggleStatus(pendingCondition!.id, pendingCondition!.condition, Math.max(1, rounds)); pendingCondition = null; }}
	onconfirmindefinite={() => { combat.toggleStatus(pendingCondition!.id, pendingCondition!.condition); pendingCondition = null; }}
