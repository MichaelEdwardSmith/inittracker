	...
	<div class="w-full max-w-lg rounded-xl border border-gray-700 bg-gray-900 shadow-2xl">
		...
		<div class="flex items-center justify-between border-b border-gray-700 px-5 py-4">
			<div class="flex items-center gap-3">
				...
				<span class="rounded-full border border-amber-600/50 bg-amber-900/40 px-2 py-0.5 text-xs font-semibold text-amber-400">BETA</span>
			</div>
		...
		<!-- Beta notice -->
		<div class="mx-5 mt-4 flex items-start gap-2.5 rounded-lg border border-amber-700/40 bg-amber-950/40 px-4 py-3">
			<svg xmlns="http://www.w3.org/2000/svg" class="mt-0.5 h-4 w-4 shrink-0 text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
				<path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/>
			</svg>
			<p class="text-xs leading-relaxed text-amber-300/90">
				Voice commands are a <strong>beta feature</strong> and may not always behave as expected. Speech recognition runs locally via Whisper AI and accuracy can vary based on microphone quality, background noise, and combatant names.
			</p>
		</div>
		...
		<div class="divide-y divide-gray-800 px-5 py-4">
			{#each sections as section}
				<div class="py-3 first:pt-0 last:pb-0">
					<p class="mb-2 text-xs font-bold tracking-widest text-gray-500 uppercase">{section.label}</p>
					<div class="space-y-1.5">
						{#each section.commands as cmd}
							<div class="flex items-baseline justify-between gap-4">
								<code class="rounded bg-gray-800 px-2 py-0.5 text-xs text-amber-300">{cmd.phrase}</code>
		...
		<div class="flex items-center justify-between border-t border-gray-700 px-5 py-3">
			<p class="text-xs text-gray-600">Use <span class="text-gray-500">"Tracker"</span> as the wake word to avoid false triggers.</p>
			<button
				onclick={onclose}
