	...
	>
		...
		<div class="flex shrink-0 items-center justify-between border-b border-gray-800 px-5 py-4">
			<div>
				...
				<p class="text-xs text-gray-600">{messages.length} message{messages.length !== 1 ? 's' : ''}</p>
			</div>
					...
					{#each [...messages].reverse() as msg (msg.id)}
						<li class="px-5 py-4">
							<div class="mb-1 flex items-center justify-between gap-2">
								...
								<span class="text-xs text-gray-600">{formatDate(msg.timestamp)} {formatTime(msg.timestamp)}</span>
							</div>
