...
>
	<div
		class="mx-4 flex max-h-[80vh] w-full max-w-md flex-col rounded-xl border border-amber-700/60 bg-gray-900 shadow-2xl"
	>
		<!-- Header -->
		...
		<!-- Item list -->
		<div class="flex flex-1 flex-col gap-2 overflow-y-auto px-5 py-3">
			{#if items.length === 0}
				<p class="py-2 text-xs text-gray-600 italic">No loot. Click "Add Item" to add manually.</p>
			{/if}
			{#each items as item (item.id)}
				<div class="flex items-center gap-2">
					...
					<input
						...
						class="flex-1 rounded border border-gray-700 bg-gray-800 px-2 py-1 text-sm text-gray-200 placeholder:text-gray-600 focus:border-amber-600 focus:outline-none"
					/>
					<button
