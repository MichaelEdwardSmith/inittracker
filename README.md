...
- **Chase Tracker** — run the DMG chase rules with a live "Gap Track" on the player display; stage quarry and pursuers from the current initiative order, then Start Chase to show numbered, colour-coded badges sliding between six distance bands (Adjacent → Out of Sight) with a name/exhaustion legend, an in-panel collapsible rules summary, drawable urban/wilderness complications, and a full-screen player takeover (minimizable, mutable) with looping music and sound cues for band changes and exhaustion
