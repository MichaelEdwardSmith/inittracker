...
- **Random Dungeon Generator** — click the **🗺 Dungeon** button in the DM header to open the generator modal; configure party size, party level, dungeon size (small/medium/large), encounter difficulty, whether to include a boss room, and the number of floors (1–5), then click **Generate Dungeon** to produce a fully drawn tile-based dungeon map; features include: named rooms with level-appropriate random encounters, tile grid lines in corridors, doors with ~25% trap chance (red frame with **!**, click for trap name/trigger/DC/effect), a 💰 loot icon in 25% of rooms (click for level-scaled coin and item rewards), ▲/▼ stair icons in room corners linking floors, a floor-selector dropdown, zoom controls, and a room-click panel showing the full encounter; the map scrolls horizontally and vertically on all screen sizes
